<template>
  <div class="mb-6">
    <label class="block text-gray-700 dark:text-slate-300 font-medium mb-3">{{ $t('components.ttl.deleteAfter') }}</label>
    <div class="flex flex-wrap gap-4">
      <label class="flex items-center cursor-pointer">
        <input type="radio" value="1h" v-model="selectedPreset" class="mr-2 text-blue-600 focus:ring-blue-500">
        <span class="text-gray-700 dark:text-slate-300">{{ $t('components.ttl.hour1') }}</span>
      </label>
      <label class="flex items-center cursor-pointer">
        <input type="radio" value="12h" v-model="selectedPreset" class="mr-2 text-blue-600 focus:ring-blue-500">
        <span class="text-gray-700 dark:text-slate-300">{{ $t('components.ttl.hours12') }}</span>
      </label>
      <label class="flex items-center cursor-pointer">
    ...
    <div v-if="isCustom" class="mt-3 p-3 bg-gray-100 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700">
      <div class="flex items-center gap-2">
        ...
        <select v-model="customUnit" class="px-3 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded text-gray-900 dark:text-slate-200 focus:outline-none focus:border-blue-500">
          <option value="minutes">{{ $t('components.ttl.minutes') }}</option>
          <option value="hours">{{ $t('components.ttl.hours') }}</option>
          <option value="days">{{ $t('components.ttl.days') }}</option>
        </select>
      </div>
    </div>
    <p class="text-gray-500 dark:text-slate-400 text-sm mt-2">{{ $t('components.ttl.expires', { preview: expirationPreview }) }}</p>
  </div>
</template>
